...
*Standalone mode is supported, as in version 2.x. However, some pin assignments have changed, so the hardware is not compatible. In particular, with an Arduino Nano3, v2.x will fit on a 30-row breadboard, but v3.x will not.*
